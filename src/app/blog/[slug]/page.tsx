...
export default async function BlogPostPage({ params }: Props) {
  ...
  return (
    <article>
      <AnimatedSection
        as="header"
        className="relative bg-muted/30 py-24 md:py-40"
        amount={0.4}
      >
            ...
            {postTags
              ...
              .map((tag) => (
                <Badge
                  key={tag}
                  variant="outline"
                  className="bg-background/30 text-sm backdrop-blur-sm"
                >
                  {tag}
                </Badge>
              ))}
      ...
      <AnimatedSection
        as="div"
        className="container prose mx-auto max-w-3xl pb-20 dark:prose-invert"
        amount={0.3}
      >
        {content}
      </AnimatedSection>
